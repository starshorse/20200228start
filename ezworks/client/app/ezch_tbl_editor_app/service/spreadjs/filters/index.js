...
/*		
		selections.map( ( sel_item )=>{
			let index_
			for( index_ = 0 ; index_ < sel_item.rowCount ; index_++ , sel_item.row++ ){ 
				let filter_rows 
				if( sel_item.col == -1 ){
					let id_index_obj = spreadJs_service.getId_index_obj()
					filter_rows = spread_data.filter((ent)=>ent[id_index_obj.key] == sheet0.getValue( sel_item.row , id_index_obj.index ) )
				}else{
					let col_title = sheet0.getValue( 0, sel_item.col , $.wijmo.wijspread.SheetArea.colHeader ) 
					let col_name = headInfos.find((ent)=>ent.displayName == col_title ).name  
					let  key  = sheet0.getValue( sel_item.row , sel_item.col )
					filter_rows = spread_data.filter((ent)=>ent[col_name] == key ) 
				}
				filter_data = filter_data.concat( filter_rows ) 
			}
		})
*/		
...
//        sheet0.setRowCount(2,$.wijmo.wijspread.SheetArea.colHeader);
	...
	this.doColFilter = ( setAutoFilter )=>{
		...
		try{
			if( appConfiguration.component.mainButtons.enable ){ 		
				if( mainButtons_service.get_fullCol_mode()  ){
					var colInfo = headInfos.filter((ent)=>ent.view_mode_1 == 1 ) 
...
//		if( colInfo == null )sheet0.bindColumns( headInfos ) 
...
//		spreadJs_service.filterSpreadjs_colInfo( colInfo ) 
