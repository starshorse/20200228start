		...
		headInfos_.forEach((ent)=>{
			...
			try{
				...
				switch( formatConfiguration.type ){
					case 'comboBox':
					    spreadJs_factory.setColumnType( ent.name , 'comboBox', formatConfiguration.select_data ) 
						break;
				    default :		
				}
			}catch(err){
...
/*		
		let sheet1 = spreadJs_factory.getSheet1() 
		let headInfos = spreadJs_factory.getHeadInfos() 
		let spread = spreadJs_factory.getSpread()
*/		
		let sheet1 = spreadJs_factory.sheet1 
		let headInfos = spreadJs_factory.headInfos 
...
//		formulas_service.applyFormulas() 
	...
	this.getSpread  = spreadJs_factory.getSpread 
	this.getHeadInfos_sheet0  = spreadJs_factory.getHeadInfos  
//Fri Mar  4 16:06:05 KST 2022
	this.saveTbl = spreadJs_factory.saveTbl 	
	this.getSheet1 = spreadJs_factory.getSheet1  
	this.getSheet0 = spreadJs_factory.getSheet0  
	this.getHeadInfos = ()=>{ return headInfos_head } 
	this.updateHead = ( headInfos_, filter_rows = null ) =>{
		spreadJs_factory.updateHead( headInfos_ , filter_rows ) 
		spreadJs_factory.setIndex() 
		formulas_service.setLast_row() 
//		formulas_service.applyFormulas() 
		setTimeout( formulas_service.applyFormulas , 5000 ) 
//Fri May 27 04:48:58 UTC 2022
	...
	this.retract_loadHead = ( DbData_1 )=>{
		...
		spreadJs_factory.setHeadInfos( headInfos ) 
		this.loadHead() 
	}
