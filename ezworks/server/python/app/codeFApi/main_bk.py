...
from codeFApi.ezch_connectedIDList import connected_id_list 
from codeFApi.ezch_add_account import account_add 
from codeFApi.ezch_accountList import account_list
from codeFApi.ezch_certManger  import cert_manager 
from codeFApi.bk.transaction_t1  import transaction_t1 , transaction_t1_foreign 
from codeFApi.bk.transaction_t2  import transaction_t2  
from codeFApi.db.init_tables import get_serviceList_codeFApi  
from codeFApi.db.account_list import insert_accountList, insert_accountTransaction , delete_accountTransaction_m, delete_accountList , get_accountList , merge_accountTr_kr , merge_accountTr_foreign  
...
def main_bk():
    ...
    for u in result: 
        ...
        if result['STATUS'] == 0:
            ...
            rtn_result['STATUS_1']['UPDATE_ACCOUNT'] = 0 
        result = update_accountTransaction( token, db_name = db_name );
        if result['STATUS'] == 0:
            merge_accountTr_kr( db_name = db_name ); 
            rtn_result['STATUS_1']['UPDATE_TR_KR'] = 0 
        result = update_accountTransaction_foreign( token,db_name = db_name );
        if result['STATUS'] == 0:
            merge_accountTr_foreign( db_name =db_name ); 
            rtn_result['STATUS_1']['UPDATE_TR_FOR'] = 0 
    rtn_result['STATUS'] = rtn_result['STATUS_1']['UPDATE_ACCOUNT'] + rtn_result['STATUS_1']['UPDATE_TR_KR'] + rtn_result['STATUS_1']['UPDATE_TR_FOR']
