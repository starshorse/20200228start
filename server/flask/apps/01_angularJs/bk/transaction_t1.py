...
transaction_path_foreign = '/v1/kr/bank/b/exchange/transaction-list'

# 기 발급된 토큰
token =''

def transaction_t1( token , body ):
...
def transaction_t1_foreign( token , body ):
    result = { 'STATUS': -1, 'ERRORMESSAGE':'', 'DATA': '' }
    # CODEF API 요청
    response_codef_api = http_sender(codef_url + transaction_path_foreign, token, body)
    if response_codef_api.status_code == 200:
        dict = json.loads(urllib.parse.unquote_plus(response_codef_api.text))
        print(urllib.parse.unquote_plus(response_codef_api.text))
        if 'data' in dict and str(dict['data']) != '{}':
