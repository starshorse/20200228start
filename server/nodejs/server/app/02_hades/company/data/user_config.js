...
exports.get_tblEditor_configName = ( company_name , id )=>{
...
/*	
	  ...
	  */
	  ...
	  let id_tbl_editor_file = `${ tbl_editor_dir}/${ id }.json`;
...
const delete_userConfig_tblEditor = ( company_name , id, config_name )=>{
	  const { user_dir , tbl_editor_dir }  = return_dir( company_name );  
	  let id_tbl_editor_file = `${ user_dir }/${ id }.json`;
	  if( fs.existsSync( id_tbl_editor_file )){
			 let tblEditor_config_list  = JSON.parse( fs.readFileSync( id_tbl_editor_file ) );
...
exports.sync_tblEdtior_configName = ( company_name, id , direction = 'tree_view' )=>{
	 let result = { STATUS: -1 , RESULT: 'fail' , ERRORMESSAGE:'' , DATA:null } 
	const { user_dir , tbl_editor_dir }  = return_dir( company_name );  
		let id_file = `${user_dir}/${ id }.json` ; 
...
exports.sync_tblEdtior_configName_allId = ( company_name , direction = 'tree_view')=>{
	const { user_dir , tbl_editor_dir }  = return_dir( company_name );  
	let file_list = fs.readdirSync(user_dir)
	console.log( file_list );
	for( i =0 ; i< file_list.length ; i++ ){
		let id = file_list[i].replace('.json','') ;
		console.log('ID: '+ id );
