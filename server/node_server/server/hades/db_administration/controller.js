...
exports.get_db_default = async( req, res )=>{
	...
	const sql_state = `
	SELECT default_database_name AS name from sys.sql_logins where name = '${ login_id }'
	`
        let result = await sql_exec_get( undefined , undefined,  undefined , sql_state );   
	return res.status(200).json(result) 
}
...
exports.update_roles_data = async( req, res )=>{
	...
	let sql_state = `REVOKE SELECT,INSERT,UPDATE,VIEW DEFINITION ON "${ tbl_name }" TO ${ role }`
	if( grant_per != '' )
	    sql_state += `; GRANT ${grant_per} ON "${tbl_name}" TO ${ role }`
    let result = await sql_exec_post( undefined , undefined, db_name , sql_state );   
	return res.status(200).json( result ) 
}
